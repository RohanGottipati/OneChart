...
import { ViewState, Session, Template, Task } from './types';
...
const MOCK_SESSIONS: Session[] = [
  {
    ...
    transcript: "Patient complains of persistent dry cough for the last 3 days. No fever, no chills. Reports mild chest tightness when coughing...",
    documents: [
      {
        id: 'd1',
        title: 'SOAP Note',
        type: 'SOAP',
        content: "# SOAP Note\n\n**Subjective**\nPatient is a 34-year-old female presenting with a dry cough x3 days.\n\n**Objective**\nLungs clear to auscultation bilaterally.\n\n**Assessment**\nAcute Bronchitis.\n\n**Plan**\nRest, fluids, OTC antitussives.",
        createdAt: new Date('2023-10-24T14:30:00')
      }
    ],
    context: "Patient has history of seasonal allergies.",
    status: 'completed',
    tasks: [
      { id: 'tk1', content: "Prescribe Tessalon Perles", status: 'pending', tag: 'Prescription', sessionId: '1' },
      { id: 'tk2', content: "Follow up in 1 week if no improvement", status: 'pending', tag: 'Follow-up', sessionId: '1' }
    ],
    addendums: []
  }
];
...
const App: React.FC = () => {
  ...
  const handleProcessSession = async (audioBlob: Blob, patientName: string, patientGender: string, templateId: string, context: string) => {
      ...
      try {
          ...
          setSessions(prev => prev.map(s => s.id === sessionId ? { ...s, status: 'draft', transcript: "Processing failed. Please check your network and try again." } : s));
      }
  };
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="mt-1 text-slate-900">Dr. Jonathan Smith</div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-500">Email</label>
                            <div className="mt-1 text-slate-900">dr.smith@hospital.org</div>
                        </div>
